package com.example.portal.examportal.controller;
...
public class ExamController {
     ...
     * Start exam attempt
     * Input: userId (in request body via StartExamRequestDTO)
     */
    @PostMapping("/exams/{examId}/start")
    public ResponseEntity<Long> startExam(@PathVariable Long examId, @RequestBody StartExamRequestDTO request) {
        if (request.getUserId() == null) {
            return ResponseEntity.badRequest().build();
        }
        Long attemptId = examService.startExam(examId, request.getUserId());
